...
app.post('/send-message', async (req, res) => {
    ...
    try {
        ...
        // balikin respon JSON biar bisa diproses di Laravel
        res.status(200).json({
            status: true,
            message: 'Pesan berhasil dikirim',
            to: number,
            id: msg.id?._serialized ?? null // kalau lib WA ada msg.id
        });
    } catch (error) {
        res.status(500).json({
            status: false,
...
// kirim file
//http://localhost:3000/send-message?id=Siswa&number=6285329860005&message=Halo+Bro
...
activateSessions();
// ✅ Endpoint untuk cek apakah nomor terdaftar
app.get('/isregister', async (req, res) => {
    try {
        const { id, number } = req.query;

        if (!id) return res.status(400).json({ error: 'Session ID wajib diisi' });
        if (!number) return res.status(400).json({ error: 'Number wajib diisi' });

        const client = sessions[id];
        if (!client) return res.status(400).json({ error: `Session "${id}" tidak ditemukan` });

        // Fungsi bantu tunggu client ready
        const waitReady = () => new Promise((resolve) => {
            if (client.info) return resolve(); // sudah siap
        ...
        res.json({ id, number, isRegistered });
    } catch (err) {
        console.error('❌ Error cek nomor:', err.message);
